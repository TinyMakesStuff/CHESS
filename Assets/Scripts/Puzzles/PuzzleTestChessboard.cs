...
public class puzzleTestChessboard : MonoBehaviour
{
    [Header("Art Settings")]
    ...
    [SerializeField] private GameObject puzzleStartScreen;
    ...
    // puzzle management vars
    // [SerializeField] private bool ispuzzle;
    [SerializeField] private SinglePlayerGameManager singlePLayerGameManager;
    [SerializeField] private puzzle currentPuzzle;
    ...
    { 
        ...
        else
        {
            ...
            puzzleStartScreen.SetActive(true);
    ...
    private void SpawnpuzzlePieces(){
        ...
        {
            // white pieces
            if (currentPuzzle.whitePieceLocations[i, 0] != -1)
            {
                chessPieces[currentPuzzle.whitePieceLocations[i, 0], currentPuzzle.whitePieceLocations[i, 1]] = SpawnSinglePiece(currentPuzzle.whitePieceIds[i], whiteTeam);
                Debug.Log("Spawned piece");
            }

            // black pieces
            if (currentPuzzle.blackPieceLocations[i, 0] != -1)
            {
                chessPieces[currentPuzzle.blackPieceLocations[i, 0], currentPuzzle.blackPieceLocations[i, 1]] = SpawnSinglePiece(currentPuzzle.blackPieceIds[i], blackTeam);    
                Debug.Log("Spawned piece");
            }
    ...
    {
        // Application.Quit();
        Destroy(GameObject.Find("Test Object To Carry Data"));
    ...
    {
        Time.timeScale = 1;
        puzzleStartScreen.SetActive(false);
    }
    ...
    {
        // Debug.Log(currentPuzzle.aiMoveOn);
        // Debug.Log(currentPuzzle.whiteMoveTo[currentPuzzle.aiMoveOn, 0] + " " + x);
        // Debug.Log(currentPuzzle.whiteMoveTo[currentPuzzle.aiMoveOn, 1] + " " + y);
        // Debug.Log(currentPuzzle.movedPieces[currentPuzzle.aiMoveOn] + " " + cp.ID);
    ...
    private void puzzleCheck(int teamNum)
    { 
        if (singlePLayerGameManager.doingPuzzle)
        {
            CheckMate(2);
